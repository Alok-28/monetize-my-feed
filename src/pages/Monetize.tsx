...
const Monetize = () => {
  ...
  const [timeAnalytics, setTimeAnalytics] = useState<{
    postsByHour: Array<{ hour: number; count: number; likes: number; engagement: number }>;
    postsByDay: Array<{ day: string; count: number; likes: number; engagement: number }>;
    likesOverTime: Array<{ date: string; likes: number; posts: number }>;
  } | null>(null);
  ...
  useEffect(() => {
    const fetchInstagramData = async () => {
      try {
        ...
        const postsByHourMap = new Map<number, { count: number; likes: number; engagement: number }>();
        const postsByDayMap = new Map<string, { count: number; likes: number; engagement: number }>();
        const likesOverTimeMap = new Map<string, { likes: number; posts: number }>();

        analytics.media.forEach((post) => {
          const hour = post.hour || 0;
          const day = post.dayOfWeek || 'Unknown';
          const date = post.date || '';
          const likes = post.like_count || 0;
          const engagement = likes + (post.comments_count || 0);

          // Posts by hour
          const hourData = postsByHourMap.get(hour) || { count: 0, likes: 0, engagement: 0 };
          postsByHourMap.set(hour, {
            count: hourData.count + 1,
            likes: hourData.likes + likes,
            engagement: hourData.engagement + engagement
          });

          // Posts by day
          const dayData = postsByDayMap.get(day) || { count: 0, likes: 0, engagement: 0 };
          postsByDayMap.set(day, {
            count: dayData.count + 1,
            likes: dayData.likes + likes,
            engagement: dayData.engagement + engagement
          });

          // Likes over time
          const dateData = likesOverTimeMap.get(date) || { likes: 0, posts: 0 };
          likesOverTimeMap.set(date, {
            likes: dateData.likes + likes,
            posts: dateData.posts + 1
          });
        });

        // Convert maps to arrays and sort
        const postsByHour = Array.from(postsByHourMap.entries())
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {!loading && !error && (
            <>
              ...
              {timeAnalytics && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-fade-up mt-4">
                  <Card className="border-border" style={{ boxShadow: 'var(--shadow-card)' }}>
                    <CardHeader>
                      <CardTitle className="text-xl">Posts by Time of Day</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={280}>
                        <BarChart data={timeAnalytics.postsByHour}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                          <XAxis 
                            dataKey="hour" 
                            className="text-xs"
                          ...
                          <Tooltip 
                            ...
                            formatter={(value: any, name: string) => {
                              if (name === 'count') return [`${value} posts`, 'Posts'];
                              if (name === 'likes') return [formatCount(value), 'Likes'];
                              if (name === 'engagement') return [formatCount(value), 'Engagement'];
                              return value;
                            }}
                          />
                      ...
                      {timeAnalytics.postsByHour.length > 0 && (() => {
                        const bestHour = timeAnalytics.postsByHour.reduce((best, current) => 
                        ...
                        return (
                          <div className="mt-4 p-3 rounded-lg bg-muted/50">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {bestHour.hour}:00 - {formatCount(bestHour.engagement)} total engagement
                            </p>
                          </div>
                        );
                      })()}
                  ...
                  <Card className="border-border" style={{ boxShadow: 'var(--shadow-card)' }}>
                    <CardHeader>
                      <CardTitle className="text-xl">Posts by Day of Week</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={280}>
                        <BarChart data={timeAnalytics.postsByDay}>
                          <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                      ...
                      {timeAnalytics.postsByDay.length > 0 && (() => {
                        const bestDay = timeAnalytics.postsByDay.reduce((best, current) => 
                          current.engagement > best.engagement ? current : best
                        );
                        return (
                          <div className="mt-4 p-3 rounded-lg bg-muted/50">
                            <p className="text-sm font-medium">Best Day to Post</p>
                            <p className="text-xs text-muted-foreground">
                              {bestDay.day} - {formatCount(bestDay.engagement)} total engagement
                            </p>
                          </div>
                        );
                      })()}
              ...
              {timeAnalytics && timeAnalytics.likesOverTime.length > 0 && (
                <Card className="border-border mt-4" style={{ boxShadow: 'var(--shadow-card)' }}>
                  <CardHeader>
                    <CardTitle className="text-xl">Likes Over Time (Last 7 Days)</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={280}>
                      <LineChart data={timeAnalytics.likesOverTime}>
                        <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                        <XAxis 
                          dataKey="date" 
                          className="text-xs"
                        ...
                        <Tooltip 
                          ...
                          formatter={(value: any, name: string) => {
                            if (name === 'likes') return [formatCount(value), 'Likes'];
                            if (name === 'posts') return [`${value} posts`, 'Posts'];
                            return value;
                          }}
                        />
                        <Line 
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                      <div className="p-3 rounded-lg bg-muted/50">
                        <p className="text-sm text-muted-foreground">Total Likes (7 days)</p>
                        <p className="text-lg font-bold">
                          {formatCount(timeAnalytics.likesOverTime.reduce((sum, day) => sum + day.likes, 0))}
                        </p>
                      </div>
                      <div className="p-3 rounded-lg bg-muted/50">
                        ...
                        <p className="text-lg font-bold">
                          {formatCount(
                            Math.round(
                              timeAnalytics.likesOverTime.reduce((sum, day) => sum + day.likes, 0) /
                              Math.max(timeAnalytics.likesOverTime.reduce((sum, day) => sum + day.posts, 0), 1)
                            )
                          )}
                        </p>
                      </div>
                      <div className="p-3 rounded-lg bg-muted/50">
                        <p className="text-sm text-muted-foreground">Total Posts (7 days)</p>
                        <p className="text-lg font-bold">
                          {timeAnalytics.likesOverTime.reduce((sum, day) => sum + day.posts, 0)}
                        </p>
                      </div>
                      <div className="p-3 rounded-lg bg-muted/50">
                        ...
                        <p className="text-lg font-bold">
                          {(() => {
                            const best = timeAnalytics.likesOverTime.reduce((best, current) => 
